...
// Theme init
if (!function_exists('pastore_church_widget_recent_posts_theme_setup')) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_widget_recent_posts_theme_setup', 1 );
	function pastore_church_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('pastore_church_action_shortcodes_list',	'pastore_church_widget_recent_posts_reg_shortcodes');
		if (function_exists('pastore_church_exists_visual_composer') && pastore_church_exists_visual_composer())
			add_action('pastore_church_action_shortcodes_list_vc','pastore_church_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('pastore_church_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'pastore_church_widget_recent_posts_load' );
	function pastore_church_widget_recent_posts_load() {
		register_widget('pastore_church_widget_recent_posts');
	}
}

// Widget Class
class pastore_church_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'pastore-church'));
		parent::__construct( 'pastore_church_widget_recent_posts', esc_html__('ThemeREX - Recent Posts', 'pastore-church'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'pastore-church')), pastore_church_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'pastore_church_sc_widget_recent_posts' ) ) {
	function pastore_church_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'pastore_church_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, pastore_church_prepare_widgets_args(pastore_church_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('pastore_church_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	pastore_church_require_shortcode("trx_widget_recent_posts", "pastore_church_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('pastore_church_widget_recent_posts_reg_shortcodes_vc')) {
	function pastore_church_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'pastore-church'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'pastore-church') ),
