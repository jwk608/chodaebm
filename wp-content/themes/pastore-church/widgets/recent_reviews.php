...
// Theme init
if (!function_exists('pastore_church_widget_recent_reviews_theme_setup')) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_widget_recent_reviews_theme_setup', 1 );
	function pastore_church_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('pastore_church_action_shortcodes_list',	'pastore_church_widget_recent_reviews_reg_shortcodes');
		if (function_exists('pastore_church_exists_visual_composer') && pastore_church_exists_visual_composer())
			add_action('pastore_church_action_shortcodes_list_vc','pastore_church_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('pastore_church_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'pastore_church_widget_recent_reviews_load' );
	function pastore_church_widget_recent_reviews_load() {
		register_widget('pastore_church_widget_recent_reviews');
	}
}

// Widget Class
class pastore_church_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'pastore-church'));
		parent::__construct( 'pastore_church_widget_recent_reviews', esc_html__('ThemeREX - Recent reviews', 'pastore-church'), $widget_ops );
...
*/
if ( !function_exists( 'pastore_church_sc_widget_recent_reviews' ) ) {
	function pastore_church_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'pastore_church_widget_recent_reviews';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_reviews' 
								. (pastore_church_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, pastore_church_prepare_widgets_args(pastore_church_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('pastore_church_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	pastore_church_require_shortcode("trx_widget_recent_reviews", "pastore_church_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('pastore_church_widget_recent_reviews_reg_shortcodes_vc')) {
	function pastore_church_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'pastore-church'),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", 'pastore-church') ),
