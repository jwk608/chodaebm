<?php
/**
 * Pastore Church Framework: social networks
 *
...
// Theme init
if (!function_exists('pastore_church_socials_theme_setup')) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_socials_theme_setup' );
	function pastore_church_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'pastore_church_facebook_og_tags', 5 );
		}
...
pastore_church_storage_set('share_links', array(
...
//	'facebook' =>		'http://www.facebook.com/sharer.php?s=100&p[url]={link}&p[title]={title}&p[summary]={descr}&p[images][0]={image}',
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=pastore_church',
...
if (!function_exists('pastore_church_add_share_link')) {
	function pastore_church_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		pastore_church_storage_set('share_links', array_merge( pastore_church_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('pastore_church_show_share_links')) {
	function pastore_church_show_share_links($args) {
		if ( pastore_church_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => pastore_church_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => pastore_church_get_custom_option('show_share'),				// share block direction
			'caption' => pastore_church_get_custom_option('share_caption'),				// share block caption
			'share' => pastore_church_get_theme_option('share_buttons'),					// list of allowed socials
		...
		$output = '<div class="sc_socials sc_socials_size_'.esc_attr($args['size']).' sc_socials_share' . ($args['type']=='drop' ? ' sc_socials_drop' : ' sc_socials_dir_' . esc_attr($args['direction'])) . '">'
			. ( '<div class="share_caption">'.($args['caption']));
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			...
			foreach ($args['share'] as $soc) {
				...
				$url = $soc['url'];
				if (empty($url)) $url = pastore_church_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(pastore_church_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div></div></div>';
...
if (!function_exists('pastore_church_prepare_socials')) {
	function pastore_church_prepare_socials($list, $style='') {
		if (empty($style)) $style = pastore_church_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || pastore_church_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : pastore_church_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('pastore_church_get_twitter_data')) {
	function pastore_church_get_twitter_data($cfg) {
...
if (!function_exists('pastore_church_prepare_twitter_text')) {
	function pastore_church_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('pastore_church_get_twitter_followers')) {
	function pastore_church_get_twitter_followers($cfg) {
		$data = pastore_church_get_twitter_data($cfg); 
...
if (!function_exists('pastore_church_get_facebook_likes')) {
	function pastore_church_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(pastore_church_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (pastore_church_strpos($headers[0], '200')) {
			$json = pastore_church_fgc($url);
...
if (!function_exists('pastore_church_facebook_og_tags')) {
	function pastore_church_facebook_og_tags() {
		global $post;
		if ( !is_singular() || pastore_church_storage_get('blog_streampage')) return;
...
if (!function_exists('pastore_church_get_feedburner_counter')) {
	function pastore_church_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(pastore_church_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (pastore_church_strpos($headers[0], '200')) {
			$xml = pastore_church_fgc($url);
