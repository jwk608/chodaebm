// Pastore Church Importer script
...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		PASTORE_CHURCH_STORAGE['importer_error_messages'] = '';
		PASTORE_CHURCH_STORAGE['importer_ignore_errors'] = true;
		pastore_church_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function pastore_church_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: PASTORE_CHURCH_STORAGE['ajax_nonce'],
		action: 'pastore_church_importer_start_import',
	...
	jQuery.post(PASTORE_CHURCH_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || PASTORE_CHURCH_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				PASTORE_CHURCH_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				pastore_church_importer_do_action(steps, idx);
			} else {
				if (PASTORE_CHURCH_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + PASTORE_CHURCH_STORAGE['importer_error_msg'] + '</h4>' + PASTORE_CHURCH_STORAGE['importer_error_messages']);
				} else {
