// Pastore Church Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.pastore_church_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.pastore_church_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && pastore_church_get_cookie('to_demo_fixed')==1) pastore_church_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.pastore_church_options_field_info .to_demo_pin', function(e) {
		pastore_church_options_fix_demo(jQuery(this).parents('.pastore_church_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	pastore_church_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.pastore_church_options_input,.pastore_church_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.pastore_church_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		pastore_church_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.pastore_church_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.pastore_church_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .pastore_church_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.pastore_church_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? PASTORE_CHURCH_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.pastore_church_options_tab').find('> .pastore_church_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="pastore_church_options_schemes_order" class="pastore_church_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function pastore_church_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.pastore_church_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.pastore_church_options').find('>.pastore_church_options_field_info');
...
function pastore_church_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.pastore_church_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	pastore_church_message_confirm(PASTORE_CHURCH_STORAGE['to_strings']['scheme_delete_confirm'], PASTORE_CHURCH_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'pastore_church_options_scheme_delete',
			nonce: PASTORE_CHURCH_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.pastore_church_options_tab').find('> .pastore_church_options_schemes_order').val()
		};
		setTimeout(function(){
			pastore_church_message_info(PASTORE_CHURCH_STORAGE['to_strings']['recompile_styles'], PASTORE_CHURCH_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(PASTORE_CHURCH_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				pastore_church_message_success(PASTORE_CHURCH_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+PASTORE_CHURCH_STORAGE['to_strings']['reload_page'], PASTORE_CHURCH_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				pastore_church_message_warning(PASTORE_CHURCH_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, PASTORE_CHURCH_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function pastore_church_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.pastore_church_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	pastore_church_message_confirm(PASTORE_CHURCH_STORAGE['to_strings']['scheme_copy_confirm'], PASTORE_CHURCH_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'pastore_church_options_scheme_copy',
			nonce: PASTORE_CHURCH_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.pastore_church_options_tab').find('> .pastore_church_options_schemes_order').val()
		};
		...
		jQuery.post(PASTORE_CHURCH_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				pastore_church_message_success(PASTORE_CHURCH_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+PASTORE_CHURCH_STORAGE['to_strings']['reload_page'], PASTORE_CHURCH_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				pastore_church_message_warning(PASTORE_CHURCH_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, PASTORE_CHURCH_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function pastore_church_options_fix_demo(info) {
	var info2 = info.parents('.pastore_church_options').toggleClass('to_demo_fixed').find('>.pastore_church_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.pastore_church_options').append(clonedObj);
		info2 = info.parents('.pastore_church_options').find('>.pastore_church_options_field_info');
	}
	pastore_church_set_cookie('to_demo_fixed', jQuery('.pastore_church_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function pastore_church_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.pastore_church_options_field_info').data('scheme');
