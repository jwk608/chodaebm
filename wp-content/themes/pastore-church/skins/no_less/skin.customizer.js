...
function pastore_church_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = PASTORE_CHURCH_STORAGE['co_add_params']!=undefined ? PASTORE_CHURCH_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#pastore_church-customizer-styles-'+option).length > 0 ? jQuery('#pastore_church-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="pastore_church-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
