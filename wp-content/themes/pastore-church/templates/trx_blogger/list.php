...
if ( !function_exists( 'pastore_church_template_list_theme_setup' ) ) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_template_list_theme_setup', 1 );
	function pastore_church_template_list_theme_setup() {
		pastore_church_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'pastore-church')
			));
	}
}

// Template output
if ( !function_exists( 'pastore_church_template_list_output' ) ) {
	function pastore_church_template_list_output($post_options, $post_data) {
