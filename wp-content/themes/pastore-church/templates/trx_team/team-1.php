...
if ( !function_exists( 'pastore_church_template_team_1_theme_setup' ) ) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_template_team_1_theme_setup', 1 );
	function pastore_church_template_team_1_theme_setup() {
		pastore_church_add_template(array(
			'layout' => 'team-1',
			'template' => 'team-1',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_bottom',*/
			'title'  => esc_html__('Team /Style 1/', 'pastore-church'),
...
// Template output
if ( !function_exists( 'pastore_church_template_team_1_output' ) ) {
	function pastore_church_template_team_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_avatar"><?php echo trim($post_options['photo']); ?><?php echo trim($post_options['socials']); ?></div>
