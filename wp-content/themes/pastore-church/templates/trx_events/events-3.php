...
if ( !function_exists( 'pastore_church_template_events_3_theme_setup' ) ) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_template_events_3_theme_setup', 1 );
	function pastore_church_template_events_3_theme_setup() {
		pastore_church_add_template(array(
			'layout' => 'events-3',
			'template' => 'events-3',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 3/', 'pastore-church')
		));
	}
}

// Template output
if ( !function_exists( 'pastore_church_template_events_3_output' ) ) {
	function pastore_church_template_events_3_output($post_options, $post_data) {
		$show_title = true;
		...
		<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
			...
			<span class="sc_events_item_details"><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?><a class="sc_button sc_button_square sc_button_style_color_1 sc_button_size_large sc_events_item_readmore" href="<?php echo esc_url($post_data['post_link']); ?>"><?php echo trim($post_options['readmore']); ?></a><?php
				}
			?></span>
		</div>
		<?php
