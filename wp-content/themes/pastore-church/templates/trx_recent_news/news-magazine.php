...
if ( !function_exists( 'pastore_church_template_news_magazine_theme_setup' ) ) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_template_news_magazine_theme_setup', 1 );
	function pastore_church_template_news_magazine_theme_setup() {
		pastore_church_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'pastore-church'),
...
// Template output
if ( !function_exists( 'pastore_church_template_news_magazine_output' ) ) {
	function pastore_church_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
			...
			if ($number <= $featured) {
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//pastore_church_get_theme_option('blog_counters');
