<?php
/**
 * Pastore Church Framework: Testimonial support
 *
...
// Theme init
if (!function_exists('pastore_church_testimonial_theme_setup')) {
	add_action( 'pastore_church_action_before_init_theme', 'pastore_church_testimonial_theme_setup', 1 );
	function pastore_church_testimonial_theme_setup() {
	
		// Add item in the admin menu
		add_action('add_meta_boxes',		'pastore_church_testimonial_add_meta_box');

		// Save data from meta box
		add_action('save_post',				'pastore_church_testimonial_save_data');

		// Register shortcodes [trx_testimonials] and [trx_testimonials_item]
		add_action('pastore_church_action_shortcodes_list',		'pastore_church_testimonials_reg_shortcodes');
		if (function_exists('pastore_church_exists_visual_composer') && pastore_church_exists_visual_composer())
			add_action('pastore_church_action_shortcodes_list_vc','pastore_church_testimonials_reg_shortcodes_vc');

		// Meta box fields
		pastore_church_storage_set('testimonial_meta_box', array(
			'id' => 'testimonial-meta-box',
			'title' => esc_html__('Testimonial Details', 'pastore-church'),
			'page' => 'testimonial',
			'context' => 'normal',
			'priority' => 'high',
			'fields' => array(
				"testimonial_author" => array(
					"title" => esc_html__('Testimonial author',  'pastore-church'),
					"desc" => wp_kses_data( __("Name of the testimonial's author", 'pastore-church') ),
					"class" => "testimonial_author",
					"std" => "",
					"type" => "text"),
				"testimonial_position" => array(
					"title" => esc_html__("Author's position",  'pastore-church'),
					"desc" => wp_kses_data( __("Position of the testimonial's author", 'pastore-church') ),
					"class" => "testimonial_author",
					"std" => "",
					"type" => "text"),
				"testimonial_email" => array(
					"title" => esc_html__("Author's e-mail",  'pastore-church'),
					"desc" => wp_kses_data( __("E-mail of the testimonial's author - need to take Gravatar (if registered)", 'pastore-church') ),
					"class" => "testimonial_email",
					"std" => "",
					"type" => "text"),
				"testimonial_link" => array(
					"title" => esc_html__('Testimonial link',  'pastore-church'),
					"desc" => wp_kses_data( __("URL of the testimonial source or author profile page", 'pastore-church') ),
		...
		pastore_church_theme_support_pt('testimonial');
		pastore_church_theme_support_tx('testimonial_group');
...
// Add meta box
if (!function_exists('pastore_church_testimonial_add_meta_box')) {
	//add_action('add_meta_boxes', 'pastore_church_testimonial_add_meta_box');
	function pastore_church_testimonial_add_meta_box() {
		$mb = pastore_church_storage_get('testimonial_meta_box');
		add_meta_box($mb['id'], $mb['title'], 'pastore_church_testimonial_show_meta_box', $mb['page'], $mb['context'], $mb['priority']);
	}
}

// Callback function to show fields in meta box
if (!function_exists('pastore_church_testimonial_show_meta_box')) {
	function pastore_church_testimonial_show_meta_box() {
		global $post;

		// Use nonce for verification
		echo '<input type="hidden" name="meta_box_testimonial_nonce" value="'.esc_attr(wp_create_nonce(admin_url())).'" />';
		
		$data = get_post_meta($post->ID, 'pastore_church_testimonial_data', true);
	
		$fields = pastore_church_storage_get_array('testimonial_meta_box', 'fields');
		?>
...
// Save data from meta box
if (!function_exists('pastore_church_testimonial_save_data')) {
	//add_action('save_post', 'pastore_church_testimonial_save_data');
	function pastore_church_testimonial_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( pastore_church_get_value_gp('meta_box_testimonial_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, 'pastore_church_testimonial_data', $data);
	}
}
...
if (!function_exists('pastore_church_sc_testimonials')) {
	function pastore_church_sc_testimonials($atts, $content=null){	
		...
		pastore_church_storage_set('sc_testimonials_data', array(
			'id' => $id,
            'style' => $style,
            'columns' => $columns,
            'counter' => 0,
		...
		$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || pastore_church_strlen($bg_texture)>2 || ($scheme && !pastore_church_param_is_off($scheme) && !pastore_church_param_is_inherit($scheme))
			...
			. '>'
			. (!empty($subtitle) ? '<h6 class="sc_testimonials_subtitle sc_item_subtitle">' . trim(pastore_church_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_testimonials_title sc_item_title">' . trim(pastore_church_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_testimonials_descr sc_item_descr">' . trim(pastore_church_strmacros($description)) . '</div>' : '')
			. (pastore_church_param_is_on($slider) 
				? ('<div class="sc_slider_swiper swiper-slider-container'
								. ' ' . esc_attr(pastore_church_get_slider_controls_classes($controls))
								. ($controls == 'side' ? ' sc_slider_pagination' : '')
		...
		if (pastore_church_param_is_on($custom) && $content) {
			...
			$args = pastore_church_query_add_posts_and_cats($args, $ids, 'testimonial', $cat, 'testimonial_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], 'pastore_church_testimonial_data', true);
		...
		return apply_filters('pastore_church_shortcode_output', $output, 'trx_testimonials', $atts, $content);
	}
	pastore_church_require_shortcode('trx_testimonials', 'pastore_church_sc_testimonials');
}
	
	
if (!function_exists('pastore_church_sc_testimonials_item')) {
	function pastore_church_sc_testimonials_item($atts, $content=null){	
		if (pastore_church_in_shortcode_blogger()) return '';
		extract(pastore_church_html_decode(shortcode_atts(array(
			// Individual params
			"author" => "",
		...
		pastore_church_storage_inc_array('sc_testimonials_data', 'counter');
	
		$id = $id ? $id : (pastore_church_storage_get_array('sc_testimonials_data', 'id') ? pastore_church_storage_get_array('sc_testimonials_data', 'id') . '_' . pastore_church_storage_get_array('sc_testimonials_data', 'counter') : '');
	
		$thumb_sizes = pastore_church_get_thumb_sizes(array('layout' => pastore_church_storage_get_array('sc_testimonials_data', 'style')));
		...
		$args = array(
			'layout' => pastore_church_storage_get_array('sc_testimonials_data', 'style'),
			'number' => pastore_church_storage_get_array('sc_testimonials_data', 'counter'),
			'columns_count' => pastore_church_storage_get_array('sc_testimonials_data', 'columns'),
			'slider' => pastore_church_storage_get_array('sc_testimonials_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			'tag_animation' => '',
			'tag_css' => $css,
			'tag_css_wh' => pastore_church_storage_get_array('sc_testimonials_data', 'css_wh'),
			'author' => $author,
			'position' => $position,
			'link' => $link,
			'email' => $email,
		...
		return apply_filters('pastore_church_shortcode_output', $output, 'trx_testimonials_item', $atts, $content);
	}
	pastore_church_require_shortcode('trx_testimonials_item', 'pastore_church_sc_testimonials_item');
}
// ---------------------------------- [/trx_testimonials] ---------------------------------------
...
if (!function_exists('pastore_church_testimonials_reg_shortcodes')) {
	//add_filter('pastore_church_action_shortcodes_list',	'pastore_church_testimonials_reg_shortcodes');
	function pastore_church_testimonials_reg_shortcodes() {
		if (pastore_church_storage_isset('shortcodes')) {

			$testimonials_groups = pastore_church_get_list_terms(false, 'testimonial_group');
			$testimonials_styles = pastore_church_get_list_templates('testimonials');
			$controls = pastore_church_get_list_slider_controls();

			pastore_church_sc_map_before('trx_title', array(
			
				// Testimonials
				"trx_testimonials" => array(
					"title" => esc_html__("Testimonials", 'pastore-church'),
					"desc" => wp_kses_data( __("Insert testimonials into post (page)", 'pastore-church') ),
					"decorate" => true,
					"container" => false,
					"params" => array(
						"title" => array(
						...
						"style" => array(
							"title" => esc_html__("Testimonials style", 'pastore-church'),
							"desc" => wp_kses_data( __("Select style to display testimonials", 'pastore-church') ),
						...
						"columns" => array(
							...
							"desc" => wp_kses_data( __("How many columns use to show testimonials", 'pastore-church') ),
							"value" => 1,
							"min" => 1,
							"max" => 6,
							"step" => 1,
						...
						"slider" => array(
							...
							"desc" => wp_kses_data( __("Use slider to show testimonials", 'pastore-church') ),
							"value" => "yes",
							"type" => "switch",
						...
						"align" => array(
							...
							"desc" => wp_kses_data( __("Alignment of the testimonials block", 'pastore-church') ),
							"divider" => true,
							"value" => "",
							"type" => "checklist",
							"dir" => "horizontal",
						...
						"custom" => array(
							...
							"desc" => wp_kses_data( __("Allow get testimonials from inner shortcodes (custom) or get it from specified group (cat)", 'pastore-church') ),
							"divider" => true,
							"value" => "no",
							"type" => "switch",
							"options" => pastore_church_get_sc_param('yes_no')
						...
						"cat" => array(
							...
							"desc" => wp_kses_data( __("Select categories (groups) to show testimonials. If empty - select testimonials from any category (group) or from IDs list", 'pastore-church') ),
							"dependency" => array(
								'custom' => array('no')
							),
							"divider" => true,
							...
							"style" => "list",
							"multiple" => true,
							"options" => pastore_church_array_merge(array(0 => esc_html__('- Select category -', 'pastore-church')), $testimonials_groups)
						),
						"count" => array(
					...
					"children" => array(
						...
						"desc" => wp_kses_data( __("Testimonials item (custom parameters)", 'pastore-church') ),
						"container" => true,
						"params" => array(
							"author" => array(
								"title" => esc_html__("Author", 'pastore-church'),
								"desc" => wp_kses_data( __("Name of the testimonmials author", 'pastore-church') ),
								"value" => "",
								"type" => "text"
							),
							"link" => array(
								"title" => esc_html__("Link", 'pastore-church'),
								"desc" => wp_kses_data( __("Link URL to the testimonmials author page", 'pastore-church') ),
								"value" => "",
								"type" => "text"
							),
							"email" => array(
								"title" => esc_html__("E-mail", 'pastore-church'),
								"desc" => wp_kses_data( __("E-mail of the testimonmials author (to get gravatar)", 'pastore-church') ),
								"value" => "",
								"type" => "text"
							),
							"photo" => array(
								"title" => esc_html__("Photo", 'pastore-church'),
								"desc" => wp_kses_data( __("Select or upload photo of testimonmials author or write URL of photo from other site", 'pastore-church') ),
								"value" => "",
								"type" => "media"
							),
							"_content_" => array(
								"title" => esc_html__("Testimonials text", 'pastore-church'),
								"desc" => wp_kses_data( __("Current testimonials text", 'pastore-church') ),
								"divider" => true,
								"rows" => 4,
								"value" => "",
								"type" => "textarea"
...
if (!function_exists('pastore_church_testimonials_reg_shortcodes_vc')) {
	//add_filter('pastore_church_action_shortcodes_list_vc',	'pastore_church_testimonials_reg_shortcodes_vc');
	function pastore_church_testimonials_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Testimonials", 'pastore-church'),
				"description" => wp_kses_data( __("Insert testimonials slider", 'pastore-church') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("Testimonials style", 'pastore-church'),
						"description" => wp_kses_data( __("Select style to display testimonials", 'pastore-church') ),
						"class" => "",
						"admin_label" => true,
					...
					array(
						...
						"description" => wp_kses_data( __("Alignment of the testimonials block", 'pastore-church') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Allow get testimonials from inner shortcodes (custom) or get it from specified group (cat)", 'pastore-church') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Select categories (groups) to show testimonials. If empty - select testimonials from any category (group) or from IDs list", 'pastore-church') ),
						...
						"class" => "",
						"value" => array_flip(pastore_church_array_merge(array(0 => esc_html__('- Select category -', 'pastore-church')), $testimonials_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
						"heading" => esc_html__("Columns", 'pastore-church'),
						"description" => wp_kses_data( __("How many columns use to show testimonials", 'pastore-church') ),
						"group" => esc_html__('Query', 'pastore-church'),
						"admin_label" => true,
						"class" => "",
						"value" => "1",
		...
		vc_map( array(
				...
				"name" => esc_html__("Testimonial", 'pastore-church'),
				"description" => wp_kses_data( __("Single testimonials item", 'pastore-church') ),
				"show_settings_on_create" => true,
				...
				"params" => array(
					array(
						...
						"description" => wp_kses_data( __("Name of the testimonmials author", 'pastore-church') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Link URL to the testimonmials author page", 'pastore-church') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"description" => wp_kses_data( __("E-mail of the testimonmials author", 'pastore-church') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "photo",
						"heading" => esc_html__("Photo", 'pastore-church'),
						"description" => wp_kses_data( __("Select or upload photo of testimonmials author or write URL of photo from other site", 'pastore-church') ),
						"class" => "",
						"value" => "",
						"type" => "attach_image"
					),
					...
					array(
						"param_name" => "content",
						"heading" => esc_html__("Testimonials text", 'pastore-church'),
						"description" => wp_kses_data( __("Current testimonials text", 'pastore-church') ),
						"class" => "",
						"value" => "",
						"type" => "textarea_html"
					),
		...
		class WPBakeryShortCode_Trx_Testimonials extends PASTORE_CHURCH_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Testimonials_Item extends PASTORE_CHURCH_VC_ShortCodeCollection {}
